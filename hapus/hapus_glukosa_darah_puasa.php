...
if (isset($_GET['id'])) {
    $id_glukosa_darah_puasa = $_GET['id'];

    // Jika ada konfirmasi penghapusan dari SweetAlert2
    if (isset($_POST['confirm_delete'])) {
        // Hapus data tekanan darah dari database
        $query = mysqli_query($koneksi, "DELETE FROM glukosa_darah_puasa WHERE id_glukosa_darah_puasa='$id_glukosa_darah_puasa'");
        ...
        if ($query) {
            ...
            echo "<script>
                Swal.fire({
                    ...
                    window.location.href = 'index.php?halaman=glukosa_darah_puasa';
                });
            </script>";
        } else {
            echo "<script src='../assets/dist/sweetalert2.all.min.js'></script>";
